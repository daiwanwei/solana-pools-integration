...
#[derive(Accounts)]
pub struct DecreaseRaydiumLiquidity<'info> {
    #[account(mut)]
...
pub fn handler<'a, 'b, 'c, 'info>(
    ...
    if liquidity == 0 && ctx.accounts.raydium_user_position.shares < liquidity {
    ...
    if ctx.accounts.raydium_user_position.shares == 0 {
        let account = ctx.accounts.raydium_user_position.to_account_info();
