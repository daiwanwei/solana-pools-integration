...
function sortTokenMints(tokenX: PublicKey, tokenY: PublicKey) {
  const [minKey, maxKey] =
    tokenX.toBuffer().compare(tokenY.toBuffer()) == 1 ? [tokenY, tokenX] : [tokenX, tokenY];
...
export function derivePresetParameter(binStep: BN, programId: PublicKey) {
  return PublicKey.findProgramAddressSync(
    [Buffer.from("preset_parameter"), new Uint8Array(binStep.toArrayLike(Buffer, "le", 2))],
    programId,
  );
}

export function derivePresetParameter2(binStep: BN, baseFactor: BN, programId: PublicKey) {
  return PublicKey.findProgramAddressSync(
    [
