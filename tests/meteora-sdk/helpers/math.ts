...
import { BASIS_POINT_MAX, MAX_BIN_PER_POSITION, SCALE_OFFSET } from "../constants";
...
export function getC(
  ...
  const currentPricePerLamport = new Decimal(1 + binStep / 10000).pow(binId.toNumber());
  ...
  const currentPriceDeltaFromMin = currentPricePerToken.sub(new Decimal(minPrice));

  const c = new Decimal(amount.toString()).mul(currentPriceDeltaFromMin.div(priceRange).pow(k));
...
export function generateAmountForBinRange(
  ...
  const minPrice = getPriceOfBinByBinId(minBinId.toNumber(), binStep).mul(toTokenMultiplier);
  const maxPrice = getPriceOfBinByBinId(maxBinId.toNumber(), binStep).mul(toTokenMultiplier);
